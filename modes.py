...
def collect_one_sign():
    ...
    shared["cur_pred"] = None; shared["cur_conf"] = 0.0   # reset display fields for this capture (3d-5)
    ...
    while time.time() - t0 < RECOG_WINDOW and shared["run"] and shared["mode"] == "recognition":
        ...
        shared["cur_pred"] = pid; shared["cur_conf"] = conf   # display only; piggybacks the call above (3d-5)
        if now - lastp > 0.5:
...
def following_step():
    ...
    shared["follow_decision"] = decision   # display only, same pattern as shared["hint"] above (3d-5)
...
# ---- read-only status aggregation for the dashboard Status tab (Phase 3d-5) ----
# Computes nothing new: hand_state() is the existing gesture-timing check;
# head_tracking_active recomputes headtracking.py's own internal gate purely
# from fields already in `shared` (no touch to headtracking.py); cur_pred/
# cur_conf/follow_decision are values collect_one_sign()/following_step()
# already compute and now also write into shared for display (see above).
def status_extra():
    l, r = hand_state()
    head_tracking_active = (shared["mode"] == "recognition"
                             and not shared["collecting"] and shared["pose_found"])
    pid = shared.get("cur_pred")
    return {
        "collecting": shared["collecting"],
        "pose_found": shared["pose_found"],
        "left_hand": l,
        "right_hand": r,
        "cur_pred_id": pid,
        "cur_pred_name": (name(pid) if pid is not None else None),
        "cur_conf": shared.get("cur_conf", 0.0),
        "follow_decision": shared.get("follow_decision"),
        "head_tracking_active": head_tracking_active,
    }

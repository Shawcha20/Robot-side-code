...
# --- aggressive anti-jitter tuning ---
DEAD_ZONE = 0.06 # was 0.14 -> reacts to smaller head movements
KP = 11        # was 8.0 -> a bit more pull 11
KD =1              # was 4.0 -> more damping to offset the higher KP 1
SMOOTH = 0.65            # was 0.8 -> slightly snappier
MAX_STEP = 6.0
MIN_MOVE = 1          # was 2.5 -> makes smaller corrections
PAN_MIN, PAN_MAX = 30, 240
TILT_MIN, TILT_MAX = 60, 210
PAN_DIR = -1            # flip to +1 if pan turns the wrong way
TILT_DIR = 1            # flip to -1 if tilt goes the wrong way
MOVE_SETTLE = 0.05      # brief pulse to start the move, then detach
...
try:
    while True:
        ...
        if res.pose_landmarks:
            ...
            # if either servo moved, give a brief pulse then cut signal (no holding jitter)
            if moved:

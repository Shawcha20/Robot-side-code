...
class Robot:
    ...
    def calibrate_now(self):
        """On-demand recalibration: whatever posture the robot is physically
        held in at this instant becomes the new pitch/roll zero reference,
        replacing the boot-time one. STM32 disables balancing + stops motors
        first (safety), then blocks ~2s measuring the held posture. Not
        deduped -- must always go out."""
        self.link.send_legacy("CAL")
